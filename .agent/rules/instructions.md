...
## プロジェクト概要
本プロジェクト **Kuniumi** (国生み) は、Go言語で記述された関数を、様々なインターフェース（HTTP, CGI, MCP, Dockerコンテナ）を通じてポータブルなWebサービスとして公開するためのフレームワークです。
...
- **仕様・設計**: `prompts/phases/` 配下が「正」の情報源です。
    - `000-firstrelease`: フェーズ名称
        - `ideas/`: 主要なアイデアメモと仕様書
            - `main/`: `main`ブランチ (現在の作業ブランチ名を入れること)
                - `000-xxx.md`: アイディアと要件を記したマークダウンファイル
                - `001-yyy.md`: 同上

## 開発プロセス

**仕様ファースト**: 実装の前に `prompts/phases/...` 以下の `ideas/` フォルダに作業ブランチ名のフォルダを作成し、その下に仕様を作成・更新してください。仕様書のファイル名は、`000-` から始まる3桁の連番の数字で管理してください。
