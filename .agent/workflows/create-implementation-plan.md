...
2.  **Integration Tests**:
    Run integration tests for the relevant categories.
    ```bash
    ./scripts/process/integration_test.sh --categories [gui|llm|taskengine|general] --specify "[Unique Test Case Name]"
    ```
...
4.  **テスト網羅性チェック (Platform Specific)**:
    ...
    *   (VSCode) E2Eテストシナリオは `TestDriver` を用いる前提になっているか。UI要素に識別子 (`data-testid`) は定義されているか。
    *   TDDで計画されているか。
